...
class ensemble_model(nn.Module):
    def __init__(self, modela, modelb, modelc):
        ...
        self.conv = nn.Sequential(
            nn.Dropout(0.1),
            nn.Linear(in_features=1121, out_features=1024, bias=False),
            nn.LeakyReLU(0.01),
            nn.Dropout(0.1),
            nn.Linear(in_features=1024, out_features=33, bias=True)
        )
...
class ensemble_model_2(nn.Module):
    def __init__(self, modela, modelb):
        ...
        self.attention = MultiHeadAttention(1536 + 576, 8)
        self.modela = modela
        self.modelb = modelb
        ...
        self.conv = nn.Sequential(
            nn.LeakyReLU(0.1),
            nn.Dropout(0.5),
            nn.Linear(in_features=2112, out_features=1024, bias=False),
            nn.LeakyReLU(0.1),
            nn.Dropout(0.5),
            nn.Linear(in_features=1024, out_features=33, bias=True)
        )
    ...
    def forward(self, x):
        ...
        caout = torch.cat((out1, out2), dim=1)
        out = self.attention(caout,caout,caout)[0].view(len(x), -1)
